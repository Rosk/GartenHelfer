...
// Interval animation
        window.setInterval(function (interval) {
            var colors = ["#4562e9", "#123456", "#aabbdd", "#ff5egd", "#cc5543", "#ef34fa"];
            var color = Math.floor((Math.random() * colors.length));
            var speed1 = 500;
            var speed2 = 750;
            /// call your function here
            interval("body").animate({
                backgroundColor: colors[color]
            }, speed2);
            interval(".navbar-inverse").animate({
                backgroundColor: colors[color],
                backgroundImage: "none"
            }, speed2, function () {
                interval(".navbar-brand").animate({
                    color: "#ffffff"
                }, speed2, function () {
                    interval(".jumbotron").animate({
                        backgroundColor: "#ffffff"
                    }, speed1);
                    interval("h2").animate({
                        color: "#ffffff"
                    }, speed1);
                    interval("section p").animate({
                        color: "rgba(255,255,255,0.7)"
                    }, speed1);
                    interval("footer p").animate({
                        color: "rgba(255,255,255,0.7)"
                    }, speed1);
                });
            });
